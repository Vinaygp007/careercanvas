...
function getCareerLink(career: string) {
  ...
  export default function ResultPage() {
    ...
    return (
    <main className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-b from-[#eaf3fc] to-white dark:from-[#0a1a2f] dark:to-black px-2 sm:px-4">
      <motion.div
        initial={{ opacity: 0, y: 40, scale: 0.96 }}
        animate={{ opacity: 1, y: 0, scale: 1 }}
        transition={{ duration: 0.8, type: 'spring' }}
        className="w-full max-w-xs sm:max-w-2xl rounded-2xl sm:rounded-3xl bg-white/90 dark:bg-gray-900/90 shadow-2xl backdrop-blur-lg px-3 sm:px-8 py-8 sm:py-14 flex flex-col items-center border border-white/40 dark:border-gray-800/60"
      >
        <h2 className="text-2xl sm:text-3xl md:text-4xl font-extrabold mb-3 sm:mb-4 text-center bg-gradient-to-r from-blue-600 via-purple-500 to-pink-500 bg-clip-text text-transparent animate-gradient-x flex items-center gap-2">
          <FaMedal className="text-yellow-400 animate-bounce" />
          Your Work Personality Profile
        </h2>
        <p className="text-base sm:text-lg text-gray-700 dark:text-gray-200 mb-4 sm:mb-6 text-center max-w-xs sm:max-w-xl">
        ...
        {unsureCount > 0 && (
          <div className="w-full flex flex-col items-center gap-2 mb-3 sm:mb-4">
            <div className={`w-full max-w-xs sm:max-w-lg rounded-xl p-3 sm:p-4 shadow text-center ${unsureRatio > 0.4 ? 'bg-red-100 text-red-700 dark:bg-red-900/40' : 'bg-yellow-50 text-yellow-700 dark:bg-yellow-900/40'}`}>
              {unsureRatio > 0.4 ? (
                <>
        ...
        {topTrait && (
          <div className="w-full flex flex-col items-center gap-2 mb-6 sm:mb-8">
            <div className="w-full max-w-xs sm:max-w-lg bg-gradient-to-r from-blue-50 to-pink-50 dark:from-blue-900/40 dark:to-pink-900/40 rounded-xl p-4 sm:p-5 shadow animate-gradient-x">
              <h3 className="text-lg sm:text-xl font-bold mb-1 sm:mb-2 text-blue-700 dark:text-pink-300 text-center">Personalized Summary</h3>
              <p className="text-sm sm:text-base text-gray-700 dark:text-gray-200 text-center">{topTrait !== undefined ? traitNarratives[topTrait] : ""}</p>
            </div>
          </div>
        )}
        <div className="w-full flex flex-col items-center gap-3 sm:gap-4 mb-6 sm:mb-8">
          <div className="flex flex-wrap gap-2 sm:gap-3 justify-center">
            {sortedTraits.slice(0, 3).map(([trait, score]) => (
              <span key={trait} className="px-3 sm:px-4 py-1.5 sm:py-2 rounded-full bg-gradient-to-r from-blue-600 to-pink-500 text-white font-semibold shadow animate-glow text-sm sm:text-base flex items-center gap-1 sm:gap-2">
                <FaStar className="text-yellow-300" />
                {traitLabels[trait as Trait]} ({score})
              </span>
            ))}
          </div>
          <div className="w-full grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-6 max-w-xs sm:max-w-lg mt-2 sm:mt-4">
            {sortedTraits.slice(0, 6).map(([trait, score]) => (
              <div key={trait} className="flex flex-col items-center bg-gradient-to-r from-blue-100 to-pink-100 dark:from-blue-900 dark:to-pink-900 rounded-xl p-3 sm:p-5 shadow animate-gradient-x w-full">
                <span className="text-base sm:text-xl font-bold text-blue-700 dark:text-pink-300 mb-0.5 sm:mb-1">{traitLabels[trait as Trait]}</span>
                <span className="text-2xl sm:text-3xl font-extrabold text-blue-600 dark:text-pink-400">{score}</span>
              </div>
            ))}
          </div>
        </div>
        <div className="w-full flex flex-col items-center gap-1 sm:gap-2 mb-6 sm:mb-8">
          <span className="text-base sm:text-lg text-gray-600 dark:text-gray-300 font-semibold flex items-center gap-1 sm:gap-2">
            <FaUserTie className="text-blue-500" /> Top Career Matches:
          </span>
          <div className="flex flex-wrap gap-2 sm:gap-3 justify-center mt-1 sm:mt-2">
            {topCareers.map((career) => (
              <a
                ...
                className="px-3 sm:px-4 py-1.5 sm:py-2 rounded-full bg-gradient-to-r from-green-400 to-blue-500 text-white font-semibold shadow animate-glow text-xs sm:text-sm flex items-center gap-1 sm:gap-2 hover:underline hover:scale-105 transition-transform"
        ...
        <div className="w-full flex flex-col items-center gap-1 sm:gap-2 mb-3 sm:mb-4">
          <span className="text-xs sm:text-base text-gray-500 dark:text-gray-400 flex items-center gap-1 sm:gap-2">
        ...
        <div className="w-full flex flex-col items-center gap-2 mt-4 sm:mt-6">
          <button
            className="flex items-center gap-1 sm:gap-2 px-4 sm:px-6 py-2 sm:py-3 rounded-lg sm:rounded-xl bg-gradient-to-r from-blue-600 to-pink-500 text-white font-bold shadow-lg hover:scale-105 transition-transform text-base sm:text-lg"
